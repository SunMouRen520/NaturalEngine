...
{
	glBindTexture(GL_TEXTURE_2D, 0);	// 确保纹理没有被绑定
...
unsigned int createRenderBufferAttachment(int width, int height)
{
	...
	glRenderbufferStorage(GL_RENDERBUFFER, GL_DEPTH24_STENCIL8, width, height);		// 为深度和模版缓冲区使用一个renderbuffer对象
	glFramebufferRenderbuffer(GL_FRAMEBUFFER, GL_DEPTH_STENCIL_ATTACHMENT, GL_RENDERBUFFER, rbo);	//现在把它链接起来
