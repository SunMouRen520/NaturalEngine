...
Terrain::Terrain(int gl)
{
	...
	glm::mat4 positionMatrix = glm::translate(id, glm::vec3(0.0, 0.0, 0.0));
	...
	shad->attachShader("Shader/terrain.vert")
...
void Terrain::setGui()
{
	...
	ImGui::TextColored(ImVec4(1, 0, 1, 1), "Terrain Controls");

	ImGui::SliderInt("Octaves", &octaves, 1, 20);
	ImGui::SliderFloat("Frequency", &frequency, 0.0f, 0.05f);
	ImGui::SliderFloat("Displancement factor", &dispFactor, 0.0f, std::pow(32.f * 32.f * 32.f, 1 / power));
	ImGui::SliderFloat("Grass coverage", &grassCoverage, 0.0f, 1.0f);
	ImGui::SliderFloat("Tessellation multiplier", &tessMultiplier, 0.1f, 5.0f);
	ImGui::SliderFloat("Fog fall-off", &fogFalloff, 0.0f, 10.0f);
	ImGui::SliderFloat("Power", &power, 0.0f, 10.0f);

	ImGui::ColorEdit3("Rock color", (float*)&rockColor[0]);
...
void Terrain::drawVertices(int nInstance)
{
	glBindVertexArray(planeVAO);
	shad->use();
	glDrawElementsInstanced(GL_PATCHES, (res - 1) * (res - 1) * 2 * 3, GL_UNSIGNED_INT, 0, nInstance);
	glBindVertexArray(0);
}
